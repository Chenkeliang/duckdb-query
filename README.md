...
| 能力 | 一句话 | 细节 |
|---|---|---|
| **文件入湖** | CSV · Excel · Parquet · JSON · JSONL | 拖进来即成表；还支持粘贴表格、URL 导入、服务器挂载目录、Excel 多工作表选择 |
| **数据库与跨源** | MySQL · PostgreSQL · SQLite · DuckDB | 通过 DuckDB `ATTACH` 与本地表一起查询，一条 SQL 跨源 JOIN |
| **查询工作流** | 写 SQL，也可以不写 | CodeMirror 编辑器、JOIN 工作台、集合运算、透视表、异步任务、查询取消、收藏与历史 |
| **数据智能体** | 用自然语言问数据 | 自主查表结构、验证真实取值、试跑只读限行查询后再回答；探查步骤可见、随时可停止 |
| **对数据库直接问数** | 本地表与远程库一起回答 | 把连接加进作用域即可提问；**只读取库结构，不加载任何数据行**，取值一律走带行数上限的只读查询；每条回答标注本轮查了哪些库 |
| **其他 AI 辅助** | 报错医生 · SQL 解释 · 图表建议 | 使用你自己配置的模型供应商与端点；AI 起草的 SQL 只插入编辑器，由你确认执行 |
| **结果与导出** | DataGrid + 6 类图表 | 虚拟滚动表格；柱状/折线/面积/饼图/环形/KPI，点击图元生成明细 SQL；导出 CSV / Excel / JSON / Parquet |
| **MCP 自动化** | 24 个工具 | 覆盖查询、发现、数据接入、转换、AI 设置与导出；`read-only` / `normal` / `full` 三种运行模式 |
