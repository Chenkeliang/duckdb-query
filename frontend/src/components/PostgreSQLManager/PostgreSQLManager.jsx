...
const PostgreSQLManager = () => {
  ...
  const loadConnections = async () => {
    try {
      setLoading(true);
      const response = await getPostgreSQLConfigs();
      if (response.success) {
        setConnections(response.configs);
      }
    } catch (err) {
      setError('加载PostgreSQL连接失败: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTestConnection = async () => {
    try {
      ...
      if (mockTestResult.success) {
        ...
        setError('连接测试失败: ' + mockTestResult.message);
      }
    } catch (err) {
  ...
  const handleSaveConnection = async () => {
    try {
      ...
      const connectionData = {
        ...
        params: {
          ...formData.params,
          port: parseInt(formData.params.port) || 5432
        }
      };
  ...
  const handleDeleteConnection = async (connectionId) => {
    if (!window.confirm('确定要删除这个PostgreSQL连接吗？')) {
      return;
    }

    try {
      setLoading(true);
      const response = await deletePostgreSQLConfig(connectionId);
      
      if (response.success) {
        setSuccess('PostgreSQL连接删除成功!');
        await loadConnections();
      } else {
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="md" fullWidth>
        <DialogTitle>
          {editingConnection ? '编辑PostgreSQL连接' : '添加PostgreSQL连接'}
        </DialogTitle>
        <DialogContent>
