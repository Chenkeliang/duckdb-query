...
本项目的Docker配置依赖正确的目录结构，请确保：

```
interactive-data-query/          # 项目根目录
├── docker-compose.yml          # 主配置文件（推荐使用）
├── api/                        # 后端代码
├── frontend/                   # 前端代码
├── config/                     # 配置文件
│   ├── mysql-configs.json      # 数据库配置
│   └── docker/                 # Docker配置目录
│       ├── docker-compose.yml  # 备用配置文件
│       └── .env.example        # 环境变量示例
└── data/                       # 数据目录
```
...
**方式2：使用config/docker配置**
```bash
cd interactive-data-query/config/docker
cp .env.example .env
# 编辑 .env 文件设置 PROJECT_ROOT=../..
docker-compose up -d
```
...
如果遇到路径错误：
1. 确认当前目录：`pwd`
2. 使用绝对路径：`PROJECT_ROOT=/absolute/path docker-compose up -d`
3. 或切换到项目根目录使用主配置文件
