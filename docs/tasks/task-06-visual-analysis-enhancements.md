...
### Phase 0：需求确认
- 对齐设计稿、交互稿与 token 规范。
- 确认旧版本配置的兼容策略（尤其是 WHERE 与计算字段的数据结构）。

### Phase 1：后端改造（先行）
1. **接口协议调整**  
   - 扩展 `filters`、`computedFields`、`having`、`windowFunctions` 四个节点的数据结构。  
   - 提供透视预览接口（示例数据）。  
2. **SQL 生成器重构**  
   - 支持列 vs 列、表达式、HAVING、CASE、窗口函数。  
   - 增加白名单校验 / 错误码输出。  
3. **兼容与迁移**  
   - 为旧配置提供迁移逻辑；补充单元测试。  

> 后端就绪后再开始大规模联调，减少前端阻塞。

### Phase 2：前端改造（依赖 Phase 1）
1. **WHERE 模式切换与条件管理**  
2. **计算字段（组合 / CASE / 窗口）与复用**  
3. **聚合与 HAVING 模块**  
4. **三列布局与折叠交互统一**  
5. **透视页面压缩 + MUI Table 预览**  
6. **状态持久化、Token 对齐、暗色主题校验**  

### Phase 3：联调 & QA
- 覆盖单元测试、接口联调、回归测试；更新文档。
...
## 实施记录
- 2025-11-05：完成 Phase 2 前端改造，覆盖 WHERE 模式切换、计算字段（组合/CASE/窗口）、HAVING 模块、三列栅格及透视预览表，并更新 utils 序列化与基础单元测试。
