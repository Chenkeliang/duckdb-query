...
| GET | `/api/duckdb/tables` | **列表** | `items[]`: `table_name`, `row_count`, `column_count`, `created_at`（应用时区 ISO；按创建时间倒序、同刻按表名，跨重启稳定；无元数据的表首见时自动回填 `created_at`——与 AI 目录同口径） | `getDuckDBTables` |
