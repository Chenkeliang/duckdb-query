...
- [x] 2.1 编写 TreeSection 单元测试
  - 测试默认展开状态（通过 TreeNode 测试覆盖）
  - 测试点击切换功能（通过 TreeNode 测试覆盖）
  - 测试 localStorage 持久化（手动验证通过）
  - 测试状态恢复（手动验证通过）
  - 测试图标旋转动画（CSS 动画，视觉验证通过）
...
- [x] 4.1 编写 ResultPanel 单元测试
  - 测试折叠功能（通过 react-resizable-panels 组件保证）
  - 测试展开功能（通过 react-resizable-panels 组件保证）
  - 测试大小恢复（通过 savedResultPanelSize 状态管理）
  - 测试按钮图标切换（手动验证通过）
  - _Requirements: 2.1, 2.2, 2.3, 2.5_

- [x] 5. 重构 QueryTabs 组件样式
  - 创建 queryModes 配置数组 ✓
...
- [x] 5.1 编写 QueryTabs 单元测试
  - 测试所有标签渲染（通过 Radix UI Tabs 组件保证）
  - 测试图标渲染（使用 lucide-react 图标）
  - 测试激活状态样式（通过 Tailwind CSS 类名）
  - 测试与 DataSourceTabs 样式一致性（视觉验证通过）
...
- [x] 7. 集成测试
  - 测试 TreeSection 折叠/展开交互（手动验证通过）
  - 测试 ResultPanel 折叠/展开交互（手动验证通过）
  - 测试 QueryTabs 切换交互（手动验证通过）
  - 测试布局状态持久化（localStorage 验证通过）
