...
class DataSourceAggregator:
    ...
    async def get_datasource(self, id: str) -> Optional[DataSourceResponse]:
        """获取单个数据源"""
        try:
            # 尝试从数据库连接中查找
            if id.startswith("db_"):
                return await self._get_database_source_by_id(id)

            # 尝试从文件数据源（DuckDB 表）中查找
            elif id.startswith("file_") or id.startswith("table_"):
                return await self._get_file_source_by_id(id)

            # 如果没有前缀，尝试所有类型
            else:
                # 先尝试数据库
                db_source = await self._get_database_source_by_id(f"db_{id}")
                if db_source:
                    return db_source

                # 再尝试文件
                file_source = await self._get_file_source_by_id(f"table_{id}")
                if file_source:
                    return file_source
    ...
    async def delete_datasource(self, id: str) -> bool:
        ...
        try:
            # 根据 ID 前缀判断数据源类型并删除
            if id.startswith("db_"):
                return await self._delete_database_source(id)
            elif id.startswith("file_") or id.startswith("table_"):
                return await self._delete_file_source(id)
            else:
                # 尝试删除数据库连接
                if await self._delete_database_source(f"db_{id}"):
                    return True
                # 尝试删除文件数据源
                if await self._delete_file_source(f"table_{id}"):
                    return True
                return False
    ...
    async def _get_database_source_by_id(
        ...
        try:
            ...
            logger.error(f"获取数据库数据源 {id} 失败: {e}")
            return None

    async def _get_file_source_by_id(self, id: str) -> Optional[DataSourceResponse]:
    ...
    async def _delete_database_source(self, id: str) -> bool:
        """删除数据库数据源"""
        try:
            # 移除 db_ 前缀
            conn_id = id.replace("db_", "")
            ...
            logger.error(f"删除数据库数据源 {id} 失败: {e}")
            return False

    async def _delete_file_source(self, id: str) -> bool:
        """删除文件数据源（DuckDB 表）"""
        try:
            # 移除 table_ 或 file_ 前缀
            table_name = id.replace("table_", "").replace("file_", "")
    ...
    def _map_connection_status(self, status) -> DataSourceStatus:
        ...
        elif status == ConnectionStatus.INACTIVE:
