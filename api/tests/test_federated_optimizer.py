...
def test_optimize_keeps_unverified_mysql_join_local():
    """Regression 2026-09-08: whole-MySQL JOIN can change comparison semantics."""
    conn = _ConnStub()
    sql = (
    ...
    assert out_sql == sql
    assert "mysql_query" not in out_sql
    assert warnings == []
