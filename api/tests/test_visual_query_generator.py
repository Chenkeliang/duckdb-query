...
class TestSQLGeneration:
    ...
    def test_aggregation_functions(self):
        ...
        config = VisualQueryConfig(
            ...
            aggregations=[
                AggregationConfig(column="amount", function=AggregationFunction.SUM, alias="total_amount"),
                AggregationConfig(column="count", function=AggregationFunction.AVG, alias="avg_count"),
                AggregationConfig(column="id", function=AggregationFunction.COUNT_DISTINCT, alias="unique_ids")
            ],
            filters=[],
            order_by=[],
            group_by=["category"]
    ...
    def test_filter_conditions(self):
        ...
        config = VisualQueryConfig(
            ...
            aggregations=[],
            filters=[
                FilterConfig(column="status", operator=FilterOperator.EQUAL, value="active"),
                FilterConfig(column="age", operator=FilterOperator.GREATER_THAN, value=18, logic_operator="AND"),
                FilterConfig(column="name", operator=FilterOperator.LIKE, value="John%", logic_operator="AND")
            ],
            order_by=[]
        )
    ...
    def test_complex_query(self):
        ...
        config = VisualQueryConfig(
            ...
            aggregations=[
                AggregationConfig(column="sales_amount", function=AggregationFunction.SUM, alias="total_sales"),
                AggregationConfig(column="order_id", function=AggregationFunction.COUNT, alias="order_count")
            ],
            filters=[
                FilterConfig(column="order_date", operator=FilterOperator.GREATER_EQUAL, value="2023-01-01"),
                FilterConfig(column="status", operator=FilterOperator.EQUAL, value="completed", logic_operator="AND")
            ],
            order_by=[
                SortConfig(column="total_sales", direction=SortDirection.DESC, priority=0)
            ],
...
class TestValidation:
    ...
    def test_between_filter_validation(self):
        ...
        config = VisualQueryConfig(
            ...
            aggregations=[],
            filters=[
                FilterConfig(column="age", operator=FilterOperator.BETWEEN, value=18, value2=None)
            ],
            order_by=[]
        )
...
class TestColumnStatistics:
    ...
    def test_get_column_statistics_success(self, mock_logger):
        ...
        mock_con.execute.return_value.fetchdf.side_effect = [
            ...
            pd.DataFrame({  # Statistics result
                'total_count': [1000],
                'non_null_count': [950],
                'null_count': [50],
                'distinct_count': [100]
            }),
            pd.DataFrame({  # Min/Max result
                'min_val': [1],
                'max_val': [100],
                'avg_val': [50.5]
            }),
            pd.DataFrame({  # Sample values
    ...
    def test_get_column_statistics_column_not_found(self):
        ...
        with pytest.raises(ValueError, match="列 'nonexistent' 在表 'test_table' 中不存在"):
...
class TestPerformanceEstimation:
    ...
    def test_estimate_simple_query_performance(self):
        ...
        mock_con = Mock()
        mock_con.execute.return_value.fetchdf.return_value = pd.DataFrame({'total_rows': [1000]})
    ...
    def test_estimate_filtered_query_performance(self):
        ...
        mock_con = Mock()
        mock_con.execute.return_value.fetchdf.return_value = pd.DataFrame({'total_rows': [10000]})
    ...
    def test_estimate_aggregated_query_performance(self):
        ...
        mock_con = Mock()
        mock_con.execute.return_value.fetchdf.return_value = pd.DataFrame({'total_rows': [100000]})
    ...
    def test_performance_warnings(self):
        ...
        mock_con = Mock()
        mock_con.execute.return_value.fetchdf.return_value = pd.DataFrame({'total_rows': [1000000]})
