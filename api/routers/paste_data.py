...
async def save_paste_data(request: PasteDataRequest):
    ...
    try:
        ...
        clean_table_name = "".join(c for c in request.table_name if c.isalnum() or c in ('_', '-')).strip()
        if not clean_table_name:
            clean_table_name = f"pasted_table_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        # 验证保存结果
        try:
            result = con.execute(f'SELECT COUNT(*) as count FROM "{clean_table_name}"').fetchone()
            saved_rows = result[0] if result else 0
            
            if saved_rows != len(df):
                logger.warning(f"保存的行数 ({saved_rows}) 与预期行数 ({len(df)}) 不匹配")
        except Exception as e:
        ...
        logger.error(f"请求数据: table_name={request.table_name}, columns={len(request.column_names)}, rows={len(request.data_rows)}")
        ...
        for table_name in tables_df["name"].tolist():
            try:
                ...
                table_info.append({
                    ...
                    "rows": row_count,
                    "schema": schema_df.to_dict('records') if not schema_df.empty else []
                })
            except Exception as e:
...
async def preview_pasted_table(table_name: str, limit: int = 100):
    ...
    try:
        ...
        # 获取表数据
        preview_df = con.execute(f'SELECT * FROM "{table_name}" LIMIT {limit}').fetchdf()
