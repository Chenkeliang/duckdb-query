        ...
        try:
            ...
            for col in df.columns:
                if df[col].dtype == 'object':
                    ...
                    print(f"DEBUG: 列 {col} 的样本值类型: {type(sample_value)}, 值: {sample_value}")
            ...
            # 直接创建表，不使用临时表
            temp_table_name = f"temp_{request.table_alias}"
            conn.register(temp_table_name, df)
            conn.execute(f'CREATE TABLE "{request.table_alias}" AS SELECT * FROM {temp_table_name}')
            ...
            # 获取表信息
            columns_result = conn.execute(f'DESCRIBE "{request.table_alias}"').fetchall()
            columns = [row[0] for row in columns_result]
            
            return {
                "success": True,
                "message": f"成功从URL读取文件并创建表: {request.table_alias}",
