...
@router.post("/api/async_query", response_model=AsyncQueryResponse, tags=["Async Tasks"])
async def submit_async_query(request: AsyncQueryRequest, background_tasks: BackgroundTasks):
    """
    提交异步查询任务
    """
    try:
...
@router.get("/api/async_tasks/{task_id}", response_model=TaskDetailResponse, tags=["Async Tasks"])
