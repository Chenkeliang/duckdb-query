...
def validate_query_config(config: VisualQueryConfig) -> ValidationResult:
    ...
    try:
        ...
        has_analysis = (
            config.selected_columns or 
            config.aggregations or 
            config.calculated_fields or 
            config.conditional_fields or 
            config.filters or 
            config.order_by or 
            config.is_distinct
        )
        ...
        for filter_config in config.filters:
            ...
            if filter_config.operator not in [FilterOperator.IS_NULL, FilterOperator.IS_NOT_NULL]:
...
def get_column_statistics(table_name: str, column_name: str, con) -> ColumnStatistics:
    ...
    try:
        ...
        if data_type.upper() in ['INTEGER', 'BIGINT', 'DOUBLE', 'FLOAT', 'DECIMAL', 'NUMERIC']:
            minmax_sql = f"""
            SELECT 
    ...
    try:
        ...
        for _, column_row in columns_df.iterrows():
            ...
            try:
                ...
                logger.warning(f"Failed to get stats for column {column_name}: {str(e)}")
