...
def create_error_response(
    ...
    """创建标准化的错误响应"""
    message, _ = ERROR_MESSAGES.get(error_code, ("未知错误", 500))
...
def analyze_error_type(error_message: str) -> ErrorCode:
    ...
    # 列不存在错误
    if "column" in error_msg_lower and "does not exist" in error_msg_lower:
        return ErrorCode.COLUMN_NOT_FOUND

    # 语法错误
    if "syntax error" in error_msg_lower or "parser error" in error_msg_lower:
        return ErrorCode.SQL_SYNTAX_ERROR

    # 类型转换错误
    if "conversion error" in error_msg_lower or "cast" in error_msg_lower:
    ...
    # 内存不足
    if "memory" in error_msg_lower or "out of memory" in error_msg_lower:
    ...
    # 约束违反
    if "constraint" in error_msg_lower or "unique" in error_msg_lower:
        return ErrorCode.CONSTRAINT_VIOLATION

    # GROUP BY错误
    if "group by" in error_msg_lower and (
    ...
    if "file too large" in error_msg_lower or "size limit" in error_msg_lower:
